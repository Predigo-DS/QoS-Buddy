...
export function ThreadIdTooltip({ threadId }: { threadId: string }) {
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger>
          <p className="rounded-md bg-background px-1 py-[2px] font-mono text-[10px] leading-[12px] tracking-tighter">
...
export function ThreadIdCopyable({
  ...
  return (
    <TooltipIconButton
      ...
      className="flex w-fit flex-grow-0 cursor-pointer items-center gap-1 rounded-md border-[1px] border-border p-1 hover:bg-background"
    >
      ...
      >
        {copied ? (
          ...
          <motion.div
            ...
            transition={{ duration: 0.15 }}
          >
            <Copy className="h-3 max-h-3 w-3 max-w-3 text-muted" />
          </motion.div>
        )}
      </AnimatePresence>
    </TooltipIconButton>
